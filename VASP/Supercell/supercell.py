...
class Atom:
	...
	def __repr__(self):
		return "%s  %f  %f  %f" % (self.symbol, self.position[0], self.position[1], self.position[2])
...
class SuperCell:
	...
	def expand_3D(self, times):
		"""
		Input how many times in each direction the structure should be repeated.
		...
		if times != 0:
			...
			for atom in self.atoms:
				for i in range(1, times + 1):
					...
					temp_atoms[-1].position[direction] = temp_atoms[-1].position[direction] + i
			self.atoms = self.atoms + temp_atoms
			self.primitive_cell.matrix[direction] = self.primitive_cell.matrix[direction] * ( times + 1)
			for atom in self.atoms:
				atom.position[direction] = atom.position[direction] / (times + 1)
			self.sort()
		else:
			pass
	...
	def sort(self):
		new_list = sorted(self.atoms, key=lambda atom: atom.symbol, reverse=True)
		self.atoms = new_list
	...
	def save_as(self, path, file_type='poscar'):
		if file_type == 'poscar':
			...
			#outfile = open(getcwd()+'/positions.xyz', 'w')
			#outfile.write(self.atom_counts(sum=True))
